...
SYSTEM_PROMPT = (
    "Você sugere tags curtas pra organizar candidatos de um sistema de RH, a partir "
    "do perfil de uma pessoa. Vai receber o perfil (formação, experiência, "
    "habilidades, certificações, nome da vaga) e a lista de tags que a empresa já "
    "usa. Responda SOMENTE com um JSON válido no formato: "
    '{"tags": [str], "interpretacao": str}. '
    "PRIORIDADE MÁXIMA: reaproveite uma tag já existente na lista sempre que ela "
    "combinar com o perfil — só invente uma tag nova quando nenhuma existente servir. "
    "Tags curtas (1-2 palavras), minúsculas, sem acento, no padrão da lista recebida "
    "(ex.: 'senior', 'remoto', 'urgente', 'bilingue'). Sugira no máximo 5. "
    "interpretacao: uma frase curta explicando por que essas tags, pra mostrar pro RH. "
    "Lista vazia se nada do perfil sugerir uma tag clara."
)


class GroqTagExtractor(_BaseGroqExtractor, ITagExtractor):
