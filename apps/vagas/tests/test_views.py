...
@pytest.mark.django_db
def test_setor_nao_pode_editar_vaga(company_factory, setor_factory, user_factory, vaga_factory):
    company = company_factory()
    setor = setor_factory(company=company)
    ...
    client = _client_for(setor_user, company)
    patch_response = client.patch(f"/v1/vagas/{vaga.id}/", {"titulo": "Novo título"})
